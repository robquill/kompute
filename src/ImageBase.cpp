...
ImageBase::~ImageBase()
{
    KP_LOG_DEBUG("Kompute ImageBase destructor started. Type: {}",
    ...
    KP_LOG_DEBUG("Kompute ImageBase destructor success");
}

void
ImageBase::destroy()
{
    KP_LOG_DEBUG("Kompute ImageBase started destroy()");

    if (!this->mDevice) {
        KP_LOG_WARN(
          "Kompute ImageBase destructor reached with null Device pointer");
        return;
    }

    if (this->mFreePrimaryImage) {
        if (!this->mPrimaryImage) {
            KP_LOG_WARN("Kompose ImageBase expected to destroy primary image "
                        "but got null image");
        } else {
            KP_LOG_DEBUG("Kompose ImageBase destroying primary image");
    ...
    if (this->mFreeStagingImage) {
        if (!this->mStagingImage) {
            KP_LOG_WARN("Kompose ImageBase expected to destroy staging image "
                        "but got null image");
        } else {
            KP_LOG_DEBUG("Kompose ImageBase destroying staging image");
            this->mDevice->destroy(
              *this->mStagingImage,
              (vk::Optional<const vk::AllocationCallbacks>)nullptr);
    ...
    if (this->mImageView) {
        KP_LOG_DEBUG("Kompose ImageBase freeing image view");
    ...
    KP_LOG_DEBUG("Kompute ImageBase successful destroy()");
}
...
ImageBase::constructDescriptorImageInfo()
{
    throw std::runtime_error(
      "ImageBase::constructDescriptorImageInfo should not be called directly - "
      "use Image or Texture override");
}

void
ImageBase::init(void* data,
            size_t dataSize,
...
void
ImageBase::recordCopyFrom(const vk::CommandBuffer& commandBuffer,
...
void
ImageBase::recordCopyFromStagingToDevice(const vk::CommandBuffer& commandBuffer)
{
    vk::ImageSubresourceLayers layer = {};
    layer.aspectMask = vk::ImageAspectFlagBits::eColor;
    layer.layerCount = 1;
...
void
ImageBase::recordCopyFromDeviceToStaging(const vk::CommandBuffer& commandBuffer)
{
    vk::ImageSubresourceLayers layer;
    layer.aspectMask = vk::ImageAspectFlagBits::eColor;
    layer.layerCount = 1;
...
void
ImageBase::recordCopyImage(const vk::CommandBuffer& commandBuffer,
...
void
ImageBase::recordCopyImageFromTensor(const vk::CommandBuffer& commandBuffer,
...
void
ImageBase::recordPrimaryMemoryBarrier(const vk::CommandBuffer& commandBuffer,
...
void
ImageBase::recordStagingMemoryBarrier(const vk::CommandBuffer& commandBuffer,
                                  vk::AccessFlags srcAccessMask,
                                  vk::AccessFlags dstAccessMask,
                                  vk::PipelineStageFlags srcStageMask,
                                  vk::PipelineStageFlags dstStageMask)
...
void
ImageBase::recordPrimaryImageBarrier(const vk::CommandBuffer& commandBuffer,
                                 vk::AccessFlags srcAccessMask,
                                 vk::AccessFlags dstAccessMask,
                                 vk::PipelineStageFlags srcStageMask,
                                 vk::PipelineStageFlags dstStageMask,
...
void
ImageBase::recordStagingImageBarrier(const vk::CommandBuffer& commandBuffer,
                                 vk::AccessFlags srcAccessMask,
                                 vk::AccessFlags dstAccessMask,
                                 vk::PipelineStageFlags srcStageMask,
                                 vk::PipelineStageFlags dstStageMask,
...
void
ImageBase::recordImageMemoryBarrier(const vk::CommandBuffer& commandBuffer,
...
ImageBase::constructDescriptorSet(vk::DescriptorSet descriptorSet, uint32_t binding)
{
...
vk::ImageUsageFlags
ImageBase::getStagingImageUsageFlags()
{
...
ImageBase::getPrimaryImage()
{
    return this->mPrimaryImage;
}

vk::ImageLayout
ImageBase::getPrimaryImageLayout()
{
    return this->mPrimaryImageLayout;
}

uint32_t
ImageBase::getNumChannels()
{
    return this->mNumChannels;
}

void
ImageBase::allocateMemoryCreateGPUResources()
{
...
void
ImageBase::createImage(std::shared_ptr<vk::Image> image,
...
void
ImageBase::allocateBindMemory(std::shared_ptr<vk::Image> image,
